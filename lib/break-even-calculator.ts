...
export function calculateBreakEven(analysis: DealAnalysis): BreakEvenAnalysis {
  ...
  const annual_shortfall = monthly_shortfall * 12;

  // Rough estimate: reducing price by $X reduces mortgage payment
  // Mortgage reduction ≈ price reduction * (1 - down_payment_percent) * monthly_factor
  ...
  // Timeline Analysis (assuming 2.5% annual rent growth)
  ...
  let primary_issue = 'None - cash flow positive';
  if (!is_currently_cash_flow_positive) {
    const issues = [
    ...
    primary_issue = issues[0].name;
  }

  // Quickest path to positive
  let quickest_path_to_positive = 'Already positive';
  if (!is_currently_cash_flow_positive) {
    const paths = [
      { path: `Increase rent by $${rent_increase_needed_dollars.toFixed(0)}`, feasibility: rent_increase_needed_percent < 10 ? 1 : rent_increase_needed_percent < 20 ? 2 : 3 },
      { path: `Reduce purchase price by $${purchase_price_reduction_needed.toFixed(0)}`, feasibility: purchase_price_reduction_percent < 5 ? 1 : purchase_price_reduction_percent < 10 ? 2 : 3 },
      { path: `Reduce expenses by $${expense_reduction_needed.toFixed(0)}/month`, feasibility: expense_reduction_percent < 10 ? 1 : expense_reduction_percent < 20 ? 2 : 3 },
      { path: `Wait ${years_to_positive_cf} years for rent growth`, feasibility: years_to_positive_cf < 3 ? 2 : years_to_positive_cf < 5 ? 3 : 4 },
    ];

    paths.sort((a, b) => a.feasibility - b.feasibility);
    quickest_path_to_positive = paths[0].path;
  }
  ...
  return {
    ...
    max_affordable_expenses: max_affordable_expenses * 12, // Annual
    expense_reduction_needed: expense_reduction_needed * 12, // Annual
